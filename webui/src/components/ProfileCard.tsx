...
export const ProfileCard: React.FC<{ profile: ProfileView; onOpen: () => void }> = ({ profile, onOpen }) => (
  <button onClick={onOpen}
    className="flex flex-col gap-2 rounded-[var(--mx-radius)] border border-[var(--mx-border)] bg-[var(--mx-surface)] p-5 text-left transition hover:border-[var(--mx-primary)] hover:bg-[var(--mx-surface-hover)]"
    style={{ boxShadow: 'var(--mx-shadow)' }}>
    <span className="mx-serif text-lg font-semibold">{profile.name}</span>
